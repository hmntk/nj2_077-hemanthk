...
async function getData() {
  let resp = await fetch(
    `https://youtube.googleapis.com/youtube/v3/videos?&key=AIzaSyDBWvLB1dxwQQaZRzTkKNcHmvq6YewBoWQ&chart=mostPopular&regionCode=IN&maxResults=10`
  );
  ...
  items.forEach(({ id }) => {
    ...
    async function showSearchResults() {
      let vData = await fetch(
        `https://youtube.googleapis.com/youtube/v3/videos?&key=AIzaSyDBWvLB1dxwQQaZRzTkKNcHmvq6YewBoWQ&part=snippet&id=${id}`
      );
      let data = await vData.json();
      // console.log(data);
